...
#+BEGIN_defn
A \(\ambientvectorfield\)-soliton is a one parameter family of immersed sub-manifolds \(\oneparamsubmanifold\), such that there exists a one-parameter family of diffeomorphisms \(\oneparamdiffeo: \submanifold \to \submanifold\) and a change of time parameter \(\time = \time(\changedtime)\) such that
\[
\changedoneparamimmersions (x) = \oneparamimmersions[\changedtime] (\oneparamdiffeo(x))
\]
satisfies
...
\end{equation}
for some function \(\timefactor: \RR \to \RR\). T
#+END_defn
...
In particular, an MCF-soliton satisfies the initial condition
\begin{equation}
